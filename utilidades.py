...
def getDistancia(Punto1, Punto2):
	...
	latitudDif= abs(Punto2[0]-Punto1[0])
	longitudRadio = abs(Punto2[1]-Punto1[1])
...
def listaPuntos(punto1, punto2, distancia):
	...
	latitudDif /= hip; longitudRadio /= hip

	(Aux1,Aux2) = (x1+latitudDif,y1+longitudRadio)
	check1 = abs(latitudDif)/latitudDif
	check2 = abs(longitudRadio)/longitudRadio
	while(x2*check1 > Aux1*check1 and x2*check2 > Aux2*check2):
		finalList.append((Aux1,Aux2))
		(Aux1,Aux2) = (Aux1+latitudDif,Aux2+longitudRadio)
